...
async (dispatch: AppDispatch, getState: GetState): Promise<void> => {
  try {
    ...
    const response = await fetch(
      `${globals.API?.prefix || ""}${globals.API?.version || ""}workflows/submit`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      throw new Error(`Workflow submission failed: ${response.status} ${response.statusText}`);
    }
    ...
    postAsyncSuccessToast(
      `${workflowType === 'reembedding' ? 'Reembedding' : 'Preprocessing'} workflow submitted. ` +
      `Estimated completion: ${Math.round(data.estimatedDurationMinutes || 2)} minutes.`
    );
    ...
    postNetworkErrorToast(`${workflowType} workflow submission failed: ${error.message}`);
...
(dispatch: AppDispatch, getState: GetState): void => {
  ...
  const pollStatus = async () => {
    try {
      const response = await fetch(
        `${globals.API?.prefix || ""}${globals.API?.version || ""}workflows/status?workflow_id=${workflowId}`,
        {
          method: "GET",
          headers: {
            "Accept": "application/json",
      ...
      if (data.status.phase === "Succeeded") {
        ...
        postAsyncSuccessToast(`${workflowType || 'Workflow'} completed successfully!`);
        
      } else if (data.status.phase === "Failed" || data.status.phase === "Error") {
        dispatch({
        ...
        dispatch(stopPollingWorkflow());
        postAsyncFailureToast(`${workflowType || 'Workflow'} failed: ${data.status.message || 'Unknown error'}`);
        ...
        if (progress) {
          ...
          console.log(`Workflow ${workflowId} progress: ${progress.percentage}% (Running)`);
        }
      }
      ...
      const state = getState();
      const retryCount = state.workflowPolling.retryCount;
      ...
      if (retryCount >= state.workflowPolling.maxRetries || 
          Date.now() - startTime > POLLING_TIMEOUT_MS) {
        dispatch(stopPollingWorkflow());
        postAsyncFailureToast("Workflow polling failed. Please check workflow status manually.");
      }
    }
  };
...
async (dispatch: AppDispatch): Promise<void> => {
  try {
    ...
    // For now, we'll just try to switch to the new embedding
...
async (dispatch: AppDispatch): Promise<void> => {
  return dispatch(submitWorkflow("reembedding", reembedParams, parentName, embName, otherSelector));
};
...
> = (reembedParams: ReembeddingParameters) =>
async (dispatch: AppDispatch): Promise<void> => {
  return dispatch(submitWorkflow("preprocessing", reembedParams));
};