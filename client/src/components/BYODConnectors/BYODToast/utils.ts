...
export function isToastDismissed(toastId: string): boolean {
  ...
  try {
    ...
    const passedForcedDates = FORCED_RETRIGGER_DATES
      .filter(date => date <= today)
      .sort((a, b) => b.getTime() - a.getTime()); // Sort descending

    if (passedForcedDates.length > 0) {
      const latestPassedDate = passedForcedDates[0];
      const latestPassedDateString = latestPassedDate.toISOString().split('T')[0];
...
export function dismissToast(toastId: string): void {
  ...
  try {
    ...
    const latestPassedDate = passedForcedDates.length > 0 ? passedForcedDates[0] : null;
    const lastForcedDatePassed = latestPassedDate ?
      latestPassedDate.toISOString().split('T')[0] : undefined;
