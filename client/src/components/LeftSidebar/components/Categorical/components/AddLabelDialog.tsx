...
import { labelPrompt, isLabelErroneous } from "./Category/components/CategoryValue/labelUtil";
...
class AddLabelDialog extends React.PureComponent<Props, ComponentState> {
  ...
  handleCreate = async (assignSelected: boolean) => {
    ...
    await dispatch(
      actions.annotationCreateLabelInCategory(metadataField, trimmed, assignSelected)
    );
    this.handleClose();
  };
  ...
  render(): JSX.Element | null {
    ...
    return (
      <AnnoDialog
        ...
        annoInput={
          <LabelInput
            // @ts-expect-error ts-migrate(2322) FIXME: Type '{ labelSuggestions: null; onChange: (value: string) => void; onSelect: (value: string) => void; inputProps: { "data-testid": string; leftIcon: string; intent: string; autoFocus: boolean; }; }' is not assignable to type 'IntrinsicAttributes & IntrinsicClassAttributes<LabelInput> & Readonly<{}>'.
