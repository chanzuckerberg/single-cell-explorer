...
def diffexp_ttest(adaptor, setA, setB, top_n=8, diffexp_lfc_cutoff=0.01, arr="X", selector_lists=False):
    ...
    # For debugging reason, let's disable the futures
    # with concurrent.futures.ThreadPoolExecutor() as tp:
    #     A = tp.submit(mean_var_cnt_fn, matrix, obs, cols, row_selector_A)
    #     B = tp.submit(mean_var_cnt_fn, matrix, obs, cols, row_selector_B)
    #     try:
    ...
    A = mean_var_cnt_fn(matrix, obs, var, cols, row_selector_A)
    B = mean_var_cnt_fn(matrix, obs, var, cols, row_selector_B)
    try:
        meanA, varA, nA = A
        meanB, varB, nB = B
    except Exception as e:
...
def mean_var_cnt_sparse(matrix, obs, var, n_var, rows):
    ...
    print("Building map - start")
    m = {}
    for i, v in enumerate(var.df().index):
        m[v] = i
    ...
    _mean_var_sparse_accumulate(var_pos, vals, n_a, u_a, M2_a)
