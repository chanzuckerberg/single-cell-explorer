...
def request_dataset_metadata_from_data_portal(data_portal_api_base: str, explorer_url: str):
    ...
    """
    headers = {"Content-Type": "application/json", "Accept": "application/json", "Host": "dataset-metadata-portal-backend.rdev.single-cell.czi.technology"}
    try:
...
def get_dataset_metadata(dataset_root: str, dataset_id: str, app_config: AppConfig) -> dict:
    ...
    """
    explorer_url_path = f"https://dataset-metadata-explorer.rdev.single-cell.czi.technology/{dataset_root}/{dataset_id}"
...
def get_dataset_and_collection_metadata(dataset_root: str, dataset_id: str, app_config: AppConfig):
    ...
    try:
        ...
        res = requests.get(f"{data_locator_base_url}/collections/{collection_id}{suffix}", headers={"Host": "dataset-metadata-portal-backend.rdev.single-cell.czi.technology"}).json()
        if not res.ok:
            log_error_response_from_data_portal(res)
        res_json = res.json()
