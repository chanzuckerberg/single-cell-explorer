...
from server.common.errors import ConfigurationError, DatasetAccessError  # type: ignore
...
class CxgDataset(Dataset):
    ...
    def __init__(self, data_locator, app_config=None):  # type: ignore
        super().__init__(data_locator, app_config)  # type: ignore
        ...
        self.lsuri_results = ImmutableKVCache(lambda key: self._lsuri(uri=key, tiledb_ctx=self.tiledb_ctx))  # type: ignore
        self.arrays = ImmutableKVCache(lambda key: self._open_array(uri=key, tiledb_ctx=self.tiledb_ctx))  # type: ignore
        self.schema = None
        self.X_approximate_distribution = None

        self._validate_and_initialize()  # type: ignore
    ...
    @staticmethod
    def set_tiledb_context(context_params):  # type: ignore
    ...
    def pre_load_validation(data_locator):  # type: ignore
        ...
        if not CxgDataset.isvalid(location):  # type: ignore
    ...
    def open(data_locator, app_config):  # type: ignore
        return CxgDataset(data_locator, app_config)  # type: ignore

    def get_about(self):  # type: ignore
        return self.about if self.about else super().get_about()  # type: ignore

    def get_title(self):  # type: ignore
        return self.title if self.title else super().get_title()  # type: ignore

    def get_corpora_props(self):  # type: ignore
        return self.corpora_props if self.corpora_props else super().get_corpora_props()  # type: ignore
    ...
    def get_path(self, *urls):  # type: ignore
        return path_join(self.url, *urls)  # type: ignore

    def _init_sparsity_and_format(self):  # type: ignore
        try:
            X = self.open_array("Xr")  # type: ignore
            self.is_1d = True
        except Exception:
            X = self.open_array("X")  # type: ignore
            self.is_1d = False
        self.is_sparse = X.schema.sparse

    @staticmethod
    def _lsuri(uri, tiledb_ctx):  # type: ignore
        def _cleanpath(p):  # type: ignore
            if p[-1] == "/":
                return p[:-1]
            else:
                return p
        ...
        result = []
        tiledb.ls(uri, lambda path, type: result.append((_cleanpath(path), type)), ctx=tiledb_ctx)  # type: ignore
        return result

    def lsuri(self, uri):  # type: ignore
        """
    ...
    def isvalid(url):  # type: ignore
        ...
        if tiledb.object_type(url, ctx=CxgDataset.tiledb_ctx) != "group":
            return False
        if tiledb.object_type(path_join(url, "obs"), ctx=CxgDataset.tiledb_ctx) != "array":  # type: ignore
            return False
        if tiledb.object_type(path_join(url, "var"), ctx=CxgDataset.tiledb_ctx) != "array":  # type: ignore
            return False
        if tiledb.object_type(path_join(url, "X"), ctx=CxgDataset.tiledb_ctx) != "array" and not (  # type: ignore
            tiledb.object_type(path_join(url, "Xr"), ctx=CxgDataset.tiledb_ctx) == "array"  # type: ignore
            and tiledb.object_type(path_join(url, "Xc"), ctx=CxgDataset.tiledb_ctx) == "array"  # type: ignore
        ):
            return False
        if tiledb.object_type(path_join(url, "emb"), ctx=CxgDataset.tiledb_ctx) != "group":  # type: ignore
            return False
        return True

    def has_array(self, name):  # type: ignore
        a_type = tiledb.object_type(path_join(self.url, name), ctx=self.tiledb_ctx)  # type: ignore
        return a_type == "array"

    def _validate_and_initialize(self):  # type: ignore
        """
        ...
        * version 0.1 -- metadata attache to cxg_group_metadata array.
        ...
        about = None
        corpora_props = None
        if self.has_array("cxg_group_metadata"):  # type: ignore
            # version >0
            gmd = self.open_array("cxg_group_metadata")  # type: ignore
        ...
        self._init_sparsity_and_format()  # type: ignore

    @staticmethod
    def _open_array(uri, tiledb_ctx):  # type: ignore
        return tiledb.open(uri, mode="r", ctx=tiledb_ctx)

    def open_X_array(self, col_wise=False):  # type: ignore
        """
        ...
        if self.is_1d and col_wise:
            return self.open_array("Xc")  # type: ignore
        elif self.is_1d and not col_wise:
            return self.open_array("Xr")  # type: ignore
        else:
            ...
            return self.open_array("X")  # type: ignore

    def open_array(self, name):  # type: ignore
        try:
            p = self.get_path(name)  # type: ignore
    ...
    def get_embedding_array(self, ename, dims=2):  # type: ignore
        array = self.open_array(f"emb/{ename}")  # type: ignore
        return array[:, 0:dims]

    def compute_diffexp_ttest(self, setA, setB, top_n=None, lfc_cutoff=None, selector_lists=False):  # type: ignore
        if top_n is None:
            top_n = self.app_config.default_dataset__diffexp__top_n
        if lfc_cutoff is None:
            lfc_cutoff = self.app_config.default_dataset__diffexp__lfc_cutoff
        return diffexp_cxg.diffexp_ttest(  # type: ignore
            adaptor=self,
            setA=setA,
            setB=setB,
            top_n=top_n,
    ...
    def get_colors(self):  # type: ignore
        if self.cxg_version == "0.0":
            return dict()
        meta = self.open_array("cxg_group_metadata").meta  # type: ignore
        return json.loads(meta["cxg_category_colors"]) if "cxg_category_colors" in meta else dict()

    def __remap_indices(self, coord_range, coord_mask, coord_data):  # type: ignore
        """
    ...
    def get_X_array(self, obs_mask=None, var_mask=None):  # type: ignore
        obs_items = pack_selector_from_mask(obs_mask)  # type: ignore
        var_items = pack_selector_from_mask(var_mask)  # type: ignore
        ...
        if self.is_sparse:
            X = self.open_X_array(col_wise=True)  # type: ignore
            if self.is_1d:
                data = X.query(order="U").multi_index[var_items]
            else:
                data = X.query(order="U").multi_index[obs_items, var_items]
            nrows, obsindices = self.__remap_indices(shape[0], obs_mask, data.get("coords", data)["obs"])  # type: ignore
            ncols, varindices = self.__remap_indices(shape[1], var_mask, data.get("coords", data)["var"])  # type: ignore
            densedata = np.zeros((nrows, ncols), dtype=self.get_X_array_dtype())  # type: ignore
            densedata[obsindices, varindices] = data[""]
            return densedata
        else:
            X = self.open_X_array()  # type: ignore
            data = X.multi_index[obs_items, var_items][""]
            return data

    def get_X_approximate_distribution(self) -> XApproximateDistribution:
        return self.X_approximate_distribution  # type: ignore

    def get_shape(self):  # type: ignore
        X = self.open_X_array()  # type: ignore
        if self.is_1d:
            Xc = self.open_X_array(col_wise=True)  # type: ignore
            return (X.shape[0], Xc.shape[0])
        else:
            return X.shape

    def get_X_array_dtype(self):  # type: ignore
        X = self.open_X_array()  # type: ignore
        return X.attr(0).dtype

    def query_var_array(self, term_name):  # type: ignore
        var = self.open_array("var")  # type: ignore
        data = var.query(attrs=[term_name])[:][term_name]
        return data

    def query_obs_array(self, term_name):  # type: ignore
        obs = self.open_array("obs")  # type: ignore
        schema = self.get_schema()  # type: ignore
        try:
    ...
    def get_obs_names(self):  # type: ignore
    ...
    def get_embedding_names(self):  # type: ignore
        with ServerTiming.time("layout.lsuri"):
            pemb = self.get_path("emb")  # type: ignore
            embeddings = [os.path.basename(p) for (p, t) in self.lsuri(pemb) if t == "array"]  # type: ignore
        if len(embeddings) == 0:
            raise DatasetAccessError("cxg matrix missing embeddings")
        return embeddings

    def _get_schema(self):  # type: ignore
        if self.schema:
            return self.schema

        shape = self.get_shape()  # type: ignore
        dtype = self.get_X_array_dtype()  # type: ignore
        ...
        annotations = {}
        for ax in ("obs", "var"):
            A = self.open_array(ax)  # type: ignore
                ...
                if "type" in type_hint:
                    ...
                    if schema["type"] == "boolean" and ax == "obs":
                        ...
                        schema["categories"] = pd.Categorical(
                            self.open_array("obs").query(attrs=[attr.name])[:][attr.name].astype("bool")  # type: ignore
                        ).categories.tolist()
        ...
        obs_layout = []
        embeddings = self.get_embedding_names()  # type: ignore
        for ename in embeddings:
            A = self.open_array(f"emb/{ename}")  # type: ignore
    ...
    def get_schema(self):  # type: ignore
        if self.schema is None:
            with self.lock:
                self.schema = self._get_schema()  # type: ignore
        return self.schema

    def annotation_to_fbs_matrix(self, axis, fields=None, num_bins=None):  # type: ignore
        with ServerTiming.time(f"annotations.{axis}.query"):
            A = self.open_array(str(axis))  # type: ignore

            try:
                data = A[:] if not fields else A.query(attrs=fields)[:]

                categorical_dtypes = []
                for c in self.get_schema()["annotations"]["obs"]["columns"]:  # type: ignore
        ...
        with ServerTiming.time(f"annotations.{axis}.encode"):
            fbs = encode_matrix_fbs(df, col_idx=df.columns, num_bins=num_bins)  # type: ignore
