...
def serialize_column(builder, typed_arr):  # type: ignore
    """Serialize NetEncoding.Column"""

    (union_type, u_value) = typed_arr
    Column.ColumnStart(builder)  # type: ignore
    Column.ColumnAddUType(builder, union_type)  # type: ignore
    Column.ColumnAddU(builder, u_value)  # type: ignore
    return Column.ColumnEnd(builder)  # type: ignore


# Serialization helper
def serialize_matrix(builder, n_rows, n_cols, columns, col_idx):  # type: ignore
    """Serialize NetEncoding.Matrix"""

    Matrix.MatrixStart(builder)  # type: ignore
    Matrix.MatrixAddNRows(builder, n_rows)  # type: ignore
    Matrix.MatrixAddNCols(builder, n_cols)  # type: ignore
    Matrix.MatrixAddColumns(builder, columns)  # type: ignore
    if col_idx is not None:
        (union_type, u_val) = col_idx
        Matrix.MatrixAddColIndexType(builder, union_type)  # type: ignore
        Matrix.MatrixAddColIndex(builder, u_val)  # type: ignore
    return Matrix.MatrixEnd(builder)  # type: ignore


def guess_at_mem_needed(matrix):  # type: ignore
...
def encode_matrix_fbs(matrix, row_idx=None, col_idx=None, num_bins=None):  # type: ignore
    """
    ...
    builder = Builder(guess_at_mem_needed(matrix))  # type: ignore
    ...
    for cidx in range(n_cols - 1, -1, -1):
        ...
        typed_arr = serialize_typed_array(builder, col, num_bins=num_bins)  # type: ignore

        # serialize the Column union
        columns.append(serialize_column(builder, typed_arr))  # type: ignore

    # Serialize Matrix.columns[]
    Matrix.MatrixStartColumnsVector(builder, n_cols)  # type: ignore
    for c in columns:
        builder.PrependUOffsetTRelative(c)
    matrix_column_vec = builder.EndVector(n_cols)

    # serialize the colIndex if provided
    cidx = None  # type: ignore
    if col_idx is not None:
        cidx = serialize_typed_array(builder, col_idx)  # type: ignore

    # Serialize Matrix
    matrix = serialize_matrix(builder, n_rows, n_cols, matrix_column_vec, cidx)  # type: ignore

    builder.Finish(matrix)
    return builder.Output()


def decode_matrix_fbs(fbs):  # type: ignore
    """
    Given an FBS-encoded Matrix, return a Pandas DataFrame the contains the data and indices.
    """

    matrix = Matrix.Matrix.GetRootAsMatrix(fbs, 0)  # type: ignore
    n_rows = matrix.NRows()
    n_cols = matrix.NCols()
    ...
    columns_index = deserialize_typed_array((matrix.ColIndexType(), matrix.ColIndex()))  # type: ignore
    ...
    for col_idx in range(0, columns_length):
        ...
        data = deserialize_typed_array(tarr)  # type: ignore
